...
다양한 방법을 찾아보다 간단한 방법을 발견했다. 가장 먼저 떠오른 접근 방식은 CCTV 장비에서 직접 제공하는 HLS 기능을 사용하는 방식이었다. 현장이 미국에 있고 PoC로 진행하다보니 현장 정보에 대해 제한된 부분이 많은 어려움이 있었지만 CCTV 자체에 Export하는 API를 지원함을 알아낼수있었다. `CCTV ID`, `Mac` 등을 통해 영상을 추출해낼수있는 방법이다.
