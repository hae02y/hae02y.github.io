...
이쿼리를 실행했을때 DB에서 동작하는 방식은 1~100020 까지의 데이터를 스캔하고, 1~100000까지의 데이터를 버린후, 나머지 100001~100020만 클라이언트에게 전달한다.
...
### 결론
정리하면 하나의 정답은 없는것같다. 서비스의 특성과 데이터 규모나 사용자의 사용패턴에 따라 가장 적절한 방식을 선택하는 것이 중요한것같다. 이번 프로젝트에서 `Mybatis` 도입으로 인해