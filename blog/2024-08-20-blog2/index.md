...
tags:
  - blog
---
회사에서 작은 사내 스터디를 시작하게 되었다. 같이 참여한 동료들은 개발팀 5명으로 1일 1커밋과 주 1회 블로그 업로드를 규칙으로 정했다. 하지만 내가 운영 중이던 블로그는 정말 구린 상태다.이전부터 기록을 제대로 해보고 싶은 생각이 있어서 다양한 플랫폼을 이용해봤지만 딱 마음에 드는 게 없었고, 현실에 타협해서 구축은 나중으로 미루고 있었지만 이번 기회에 블로그 이동부터 몇 회의 블로그 업로드를 해결할 수 있는 기회를 갖게 됐다.
...
1. **벨로그(Velog)**  
    심플하고 직관적이다. 마크다운 지원이 너무 좋고 글 작성 시 바로 옆에 어떻게 작성될지 보이는 점이 큰 장점이다. 하지만 카테고리 기능이 없고 커스텀 불가능
...
1. **Free**: 깃헙 페이지는 무료로 제공되기 때문에 비용 부담이 없다.
...
2. 초고속 페이지 로딩
   - Docusaurus는 페이지 로딩이 정말 빠르다. 사용자가 클릭하는 순간 페이지가 눈 깜짝할 사이에 열린다. 노션으로 블로그를 작성할때 느꼈던 어려움은 해결!
